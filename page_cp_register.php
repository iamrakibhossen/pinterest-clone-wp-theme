...
if ($http_post) {
	if (isset($_GET['action']) && $_GET['action'] == 'resend') {
		if (empty($_POST['user_email'])) {
			$resend_status = __('<strong>ERROR</strong>: Enter email address.', 'lekhook');
		} else {
			if (of_get_option('captcha_public') != '' && of_get_option('captcha_private') != '') {
				...
				if (!$resp->success) {
					$resend_status = __('<strong>ERROR</strong>: Incorrect Captcha.', 'lekhook');
				} else {
					...
					if ($user) {
						...
						if ($verify_email != '') {
							...
							$message  = sprintf(__('Thank you for registering with %s.', 'lekhook'), $blogname) . "\r\n\r\n";
							$message .= sprintf(__('Username: %s', 'lekhook'), $user->user_login) . "\r\n\r\n";
							...
							wp_mail($user->user_email, sprintf(__('[%s] Account Registration', 'lekhook'), $blogname), $message);
		
							$resend_status = 'success';
						} else {
							$resend_status = __('<strong>ERROR</strong>: Account is already activated.', 'lekhook');
						}
					} else {
						$resend_status = __('<strong>ERROR</strong>: Email not found.', 'lekhook');
					}	
				}
			} else {
				$user = get_user_by('email', sanitize_email($_POST['user_email']));
		...
		$errors = lekhook_register_new_user($user_login, $user_email);
...
function lekhook_register_new_user( $user_login, $user_email ) {
	$errors = new WP_Error();
	...
	if ( $sanitized_user_login == '' ) {
		$errors->add( 'empty_username', __( '<strong>ERROR</strong>: Please enter a username.', 'lekhook' ) );
	} elseif ( ! validate_username( $user_login ) ) {
		$errors->add( 'invalid_username', __( '<strong>ERROR</strong>: This username is invalid because it uses illegal characters. Please enter a valid username.', 'lekhook' ) );
		$sanitized_user_login = '';
	} elseif ( username_exists( $sanitized_user_login ) ) {
		$errors->add( 'username_exists', __( '<strong>ERROR</strong>: This username is already registered. Please choose another one.', 'lekhook' ) );
	}

	// Check the e-mail address
	if ( $user_email == '' ) {
		$errors->add( 'empty_email', __( '<strong>ERROR</strong>: Please type your e-mail address.', 'lekhook' ) );
	} elseif ( ! is_email( $user_email ) ) {
		$errors->add( 'invalid_email', __( '<strong>ERROR</strong>: The email address isn&#8217;t correct.', 'lekhook' ) );
		$user_email = '';
	} elseif ( email_exists( $user_email ) ) {
		$errors->add( 'email_exists', __( '<strong>ERROR</strong>: This email is already registered, please choose another one.', 'lekhook' ) );
	}
	
	//edited: added to check the passwords
	if ( $_POST['pass1'] == '' ) {
		$errors = new WP_Error('password_blank', __('Password cannot be blank.', 'lekhook', 'lekhook'));
	}
	if ( strlen( $_POST['pass1'] ) < 6 ) {
	$errors->add('password_too_short', "<strong>ERROR</strong>: Passwords must be at least 6 characters long", 'lekhook');
	}
	if ( isset($_POST['pass1']) && $_POST['pass1'] != $_POST['pass2'] ) {
		$errors = new WP_Error('password_reset_mismatch', __('The passwords do not match.', 'lekhook'));
	}
	...
	if (of_get_option('captcha_public') != '' && of_get_option('captcha_private') != '') {
		...
		if (!$resp->success) {
			$errors = new WP_Error('incorrect_captcha', __('<strong>ERROR</strong>: Incorrect Captcha.', 'lekhook'));
		}
	}
	
	//edited: check if is spam user
	if (trim($_POST['anti-spam']) != date('Y') || empty($_POST['anti-spam']) || !empty( $_POST['anti-spam-e-email-url'])) {
		$errors = new WP_Error('password_reset_mismatch', __('Antispam field is incorrect.', 'lekhook'));
	}
	...
	if ( ! $user_id || is_wp_error( $user_id ) ) {
		$errors->add( 'registerfail', sprintf( __( '<strong>ERROR</strong>: Couldn&#8217;t register you&hellip; please contact the <a href="mailto:%s">webmaster</a> !', 'lekhook' ), get_option( 'admin_email' ) ) );
		return $errors;
	}
	...
	$message  = sprintf(__('Thank you for registering with %s.', 'lekhook'), $blogname) . "\r\n\r\n";
	$message .= sprintf(__('Username: %s', 'lekhook'), $sanitized_user_login) . "\r\n";
	$message .= sprintf(__('Password: %s', 'lekhook'), $mask_password) . "\r\n\r\n";
	$message .= __('Please click the link to verify your email:', 'lekhook') . "\r\n";
	$message .= sprintf('%s?email=verify&login=%s&key=%s', home_url('/login/'), rawurlencode($sanitized_user_login), $verify_email);

	wp_mail($user_email, sprintf(__('[%s] Account Registration', 'lekhook'), $blogname), $message);
	
	//notify admin when new user register
	//$message  = sprintf(__('New user registration on your site %s:', 'lekhook'), $blogname) . "\r\n\r\n";
	//$message .= sprintf(__('Username: %s', 'lekhook'), $sanitized_user_login) . "\r\n\r\n";
	//$message .= sprintf(__('E-mail: %s', 'lekhook'), $user_email) . "\r\n";
	...
	<div class="row">
		...
		<div class="col-sm-8 usercp-wrapper">
			<?php if (isset($_GET['action']) && $_GET['action'] == 'resend') { ?>
				<h1><?php _e('Resend Activation Email', 'lekhook') ?></h1>
				
				<?php if ($resend_status && $resend_status == 'success') { ?>
				<div class="error-msg"><div class="alert alert-success"><strong><?php _e('Please check your email for activation.', 'lekhook'); ?></strong></div></div>
				...
				<form id="resendform" action="<?php echo home_url('/signup/?action=resend'); ?>" method="post">
					<div class="form-group">
						<label class="form-label" for="user_email"><?php _e('Email', 'lekhook'); ?></label>
					...
					<input type="submit" class="btn btn-success btn-block btn-lekhook-custom" name="wp-submit" id="wp-submit" value="<?php _e('Resend', 'lekhook'); ?>" />
	
					<br />
					<p class="moreoptions"><?php _e('Check your junk/spam folder if you did not receive the activation email', 'lekhook'); ?></p>
				</form>
			<?php } else { ?>			
				<?php 
				...
				<h1<?php if (function_exists('wsl_activate')) echo ' style="border: none"'; ?>><?php _e('Sign Up', 'lekhook') ?></h1>
				...
				<form name="registerform" id="registerform" action="<?php echo home_url('/signup/'); ?>" method="post">
					<div class="form-group">
						<label class="form-label" for="user_login"><?php _e('Username', 'lekhook'); ?></label>
					...
					<?php if (of_get_option('register_agree') != '0') { ?>
						<i id="register_agree" class="fa fa-square-o fa-fw fa-lg pull-left"></i>
						<div id="register_agree_msg">
							<?php _e('I Agree To The', 'lekhook'); ?>
							<a onClick="window.open('<?php echo get_permalink(of_get_option('register_agree')); ?>','','resizable=1,scrollbars=1,top=0,left=0,width=640,height=480'); return false;" href="<?php echo get_permalink(of_get_option('register_agree')); ?>" target="_blank">
								<strong><?php _e('Terms of Service', 'lekhook'); ?></strong>
							</a>
						</div>
					<?php } ?>
					<p></p>
					<input class="btn btn-success btn-block btn-lekhook-custom"<?php if (of_get_option('register_agree') != '0') echo ' disabled="disabled"'; ?> type="submit" name="wp-submit" id="wp-submit" value="<?php _e('Sign Up', 'lekhook'); ?>" tabindex="50" />

					<br />
					<p class="text-center">
					<a class="btn btn-grey" href="<?php echo home_url('/login/'); ?>"><?php _e('Already have an account? Login now', 'lekhook'); ?></a>
					</p>
					
					<p class="text-center">
					<a class="btn btn-grey" href="<?php echo home_url('/signup/?action=resend'); ?>"><?php _e('Resend activation email', 'lekhook'); ?></a>
					</p>
				</form>
			<?php } ?>
		</div>
