...
<div class="container">
	<div class="row subpage-title">
		<h1><?php _e('Top Users (Most Followers)', 'lekhook'); ?></h1>
		<br />
	</div>
</div>
...
if ($top_user_follower_query->total_users > 0) {
	...
	foreach ($top_user_follower_query->results as $top_user_follower) {
		...
		<div class="follow-wrapper">
			...
			<button class="btn btn-success btn-block follow lekhook-follow<?php $parent_board = get_user_meta($top_user_follower->ID, '_Board Parent ID', true); if ($followed = lekhook_followed($parent_board)) { echo ' disabled'; } ?>" data-author_id="<?php echo $top_user_follower->ID; ?>" data-board_id="<?php echo $parent_board; ?>" data-board_parent_id="0" data-disable_others="no" type="button"><?php if (!$followed) { _e('Follow', 'lekhook'); } else { _e('Unfollow', 'lekhook'); } ?></button>
			</span>
			<?php } else { ?>
			<a class="btn btn-success btn-block follow" disabled="disabled"><?php _e('Myself!', 'lekhook'); ?></a>
			<?php } ?>
		</div>
	<?php 
	}
...
<div class="container">
	<div class="row subpage-title">
		<br /><br />
		<h1><?php _e('Top Users (Most Posts)', 'lekhook'); ?></h1>
		<br />
	</div>
</div>
	...
	foreach ($top_user_postcount_query->results as $top_user_postcount) {
		...
		<div class="follow-wrapper">
			...
			<a class="follow-user-name" title="<?php echo esc_attr($top_user_postcount->display_name); ?>" href="<?php echo home_url('/' . $wp_rewrite->author_base . '/') . $top_user_postcount->user_nicename; ?>/">
				...
				<p class="follow-user-meta"><?php $posts_count = count_user_posts($top_user_postcount->ID); echo $posts_count; ?> <?php if ($posts_count == 1) _e('Post', 'lekhook'); else _e('Posts', 'lekhook'); ?> &#8226; <?php if ('' == $followers_count = get_user_meta($top_user_postcount->ID, '_Followers Count', true)) echo '0'; else echo $followers_count; ?> <?php if ($followers_count == 1) _e('Follower', 'lekhook'); else _e('Followers', 'lekhook'); ?></p>
				<div class="clearfix"></div>
			</a>
			...
			?>
			<span class="undisable_buttons">
				<button class="btn btn-success btn-block follow lekhook-follow<?php $parent_board = get_user_meta($top_user_postcount->ID, '_Board Parent ID', true); if ($followed = lekhook_followed($parent_board)) { echo ' disabled'; } ?>" data-author_id="<?php echo $top_user_postcount->ID; ?>" data-board_id="<?php echo $parent_board; ?>" data-board_parent_id="0" data-disable_others="no" type="button"><?php if (!$followed) { _e('Follow', 'lekhook'); } else { _e('Unfollow', 'lekhook'); } ?></button>
			</span>
			<?php } else { ?>
			<a class="btn btn-success btn-block follow" disabled="disabled"><?php _e('Myself!', 'lekhook'); ?></a>
			<?php } ?>
