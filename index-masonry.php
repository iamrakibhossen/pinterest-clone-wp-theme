			...
			<div class="container">
				<div class="row">
					<div class="bigmsg">
						...
						<h4><?php _e('Start following some users or boards to fill this space up', 'lekhook'); ?></h4>
						<?php } else { ?>
	...
	<?php if(function_exists('wp_pagenavi') && $_GET['view'] != sanitize_title(__('likes', 'lekhook'))) { ?>
	...
	<div id="navigation">
		<ul class="pager">
			<?php if (isset($_GET['view']) && $_GET['view'] == sanitize_title(__('likes', 'lekhook'))) { //from author page - likes section ?>
				<?php
				...
				<div id="navigation">
					<ul class="pager">
						...
						<li id="navigation-previous">
							<a href="<?php echo home_url('/' . $wp_rewrite->author_base . '/' . $username . '/?view=' . sanitize_title(__('likes', 'lekhook'))); ?>&amp;pnum=<?php echo $pnum-1; ?>"><?php _e('&laquo; Previous', 'lekhook'); ?></a>
						</li>
						<?php } ?>
						
						<?php if ($maxpage != 1 && $maxpage != $pnum) { ?>
						<li id="navigation-next">
							<a href="<?php echo home_url('/' . $wp_rewrite->author_base . '/' . $username . '/?view=' . sanitize_title(__('likes', 'lekhook'))); ?>&amp;pnum=<?php echo $pnum+1; ?>"><?php _e('Next &raquo;', 'lekhook'); ?></a>
						</li>
						<?php } ?>
					</ul>
				</div>
				<?php } ?>
			<?php } else { ?>
				<li id="navigation-previous"><?php previous_posts_link(__('&laquo; Previous', 'lekhook')); ?></li>
				<li id="navigation-next"><?php next_posts_link(__('Next &raquo;', 'lekhook')); ?></li>			
			<?php } ?>
		</ul>
	</div>
	<?php } ?>
