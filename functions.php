...
function lekhook_after_setup_theme() {
	load_theme_textdomain('lekhook', get_template_directory() . '/languages');
...
add_action('after_setup_theme', 'lekhook_after_setup_theme');
...
//Action: widgets_init
function lekhook_widgets_init() {
	register_sidebar(array('id' => 'sidebar-r-t', 'name' => 'Right Sidebar for Single Posts Only (Above Boards)', 'before_widget' => '<div class="sidebar-wrapper"><div class="sidebar-inner">', 'after_widget' => '</div></div>', 'before_title' => '<h4>', 'after_title' => '</h4>'));
	register_sidebar(array('id' => 'sidebar-r', 'name' => 'Right Sidebar for Single Posts Only (Below Boards)', 'before_widget' => '<div class="sidebar-wrapper"><div class="sidebar-inner">', 'after_widget' => '</div></div>', 'before_title' => '<h4>', 'after_title' => '</h4>'));
...
add_action('widgets_init', 'lekhook_widgets_init');


//Action: wp_head
function lekhook_head() {
	//Opengraph
	if (is_single()) {
		global $post;
		setup_postdata($post);
		...
		if ($post->post_content == '') {
			...
			$output .= '<meta property="og:description" content="' . esc_attr(__('Postned onto', 'lekhook') . ' ' . $meta_board_name . ' ' . __('Board in', 'lekhook') . ' ' . $meta_category_name . ' ' . __('Category', 'lekhook')) . '" />' . "\n";
		} else {
	...
	if (is_author()) {
		...
		$avatar_id = get_user_meta($user_info->ID, 'lekhook_user_avatar', true);
...
add_action('wp_head', 'lekhook_head');


//Remove hentry from post class
function lekhook_post_class($classes) {
	$classes = array_diff($classes, array('hentry'));
	return $classes;
}
add_filter('post_class','lekhook_post_class');


//Filter: query_vars
function lekhook_query_vars($aVars) {
...
add_filter('query_vars', 'lekhook_query_vars');


//Filter: rewrite_rules_array
function lekhook_rewrite_rules_array($aRules) {
	//Rewrite source page template slug from /source/?domain=google.com to /source/google.com/
	$aNewRules = array('source/([^/]+)/?$' => 'index.php?pagename=source&domain=$matches[1]');
	$aRules = $aNewRules + $aRules;
	return $aRules;
}
add_filter('rewrite_rules_array', 'lekhook_rewrite_rules_array');
...
add_action('wp', 'lekhook_wp', 0);


//Remove canonical links for source page for Yoast SEO
if (function_exists('wpseo_init')) {
	function lekhook_wpseo_canonical ($canonical) {
	...
	add_filter('wpseo_canonical', 'lekhook_wpseo_canonical');
}


//Rewrite titles
function lekhook_wp_title( $title, $sep ) {
	if (is_tax('board')) {
		global $post;
		$user_info = get_user_by('id', $post->post_author);
		return str_replace(' Boards', '' ,$title) . ' ' . __('Board by', 'lekhook') . ' ' . $user_info->display_name;
	}
	
	if (is_page('source')) {
		global $wp_query;
		return __('Posts from', 'lekhook') . ' ' . $wp_query->query_vars['domain'] . str_replace('Source ', ' ', $title);
	}
	...
	if (is_tag()) {
		$title = __('', 'lekhook') . ' ' .$title;
	}
	
	if (is_category()) {
		$title = __('Category:', 'lekhook') . ' ' .$title;
	}
	
	if (is_search()) {
		return __('Search results for', 'lekhook') . ' ' . get_search_query();
	}
	
	return $title;
}
add_filter('wp_title', 'lekhook_wp_title', 10, 2);
...
add_action('admin_init', 'lekhook_admin_init', 1);


//Action: login_init
function lekhook_login_init() {
...
add_action('login_init', 'lekhook_login_init', 1);


//Redirect login page from wp-login.php to /login/
function lekhook_login_url($login_url, $redirect){
...
add_filter('login_url', 'lekhook_login_url', 10, 2);


//Action: wp_login_failed
function lekhook_login_failed($username) {
...
add_action('wp_login_failed', 'lekhook_login_failed');


//Ajax login
function lekhook_ajax_login() {
	$nonce = $_POST['nonce'];
...
add_action('wp_ajax_nopriv_lekhook-ajax-login', 'lekhook_ajax_login');


//Filter: wp_authenticate_user
function lekhook_wp_authenticate_user($userdata) {
	//Check whether user verified their email
	$verify = get_user_meta($userdata->ID, '_Verify Email', true);
	//user with verified email do not have this usermeta field
	if ($verify != '') {
		return new WP_Error('email_unverified', __('Email not verified. Please check your email for verification link.', 'lekhook'));
	}
	
	//check if captcha is correct
	if ($_POST['formname'] == 'lekhook_loginform' && of_get_option('captcha_public') != '' && of_get_option('captcha_private') != '') {
		...
		if (!$resp->success) {
			return new WP_Error('incorrect_captcha', __('<strong>ERROR</strong>: Incorrect Captcha.', 'lekhook'));
		}
	}
	return $userdata;
}
add_filter('wp_authenticate_user', 'lekhook_wp_authenticate_user', 1);


//Filter: authenticate
function lekhook_authenticate($user, $username, $password) {
	//Allow login using email
...
add_filter('authenticate', 'lekhook_authenticate', 20, 3);


//Add user data after successful registration
function lekhook_user_register($user_id) {
		...
		foreach($boards_name as $board_name) {
			...
			wp_insert_term (
				...
				array(
					...
					'slug' => $board_name . '__lekhookboard'
				)
			);
			$count++;
		}
	...
	/* if (of_get_option('auto_default_follows')) {
		...
		foreach ($default_follows as $default_follow) {
			...
			//if ($_POST['lekhook_follow'] == 'follow') {		
	...
	if (stripos($user_info->user_email, '@example.com') === false) {
		update_user_meta($user_id, 'lekhook_user_notify_likes', '1');
		update_user_meta($user_id, 'lekhook_user_notify_reposts', '1');
		update_user_meta($user_id, 'lekhook_user_notify_follows', '1');
		update_user_meta($user_id, 'lekhook_user_notify_comments', '1');
	} else {
		update_user_meta($user_id, 'lekhook_user_notify_likes', '0');
		update_user_meta($user_id, 'lekhook_user_notify_reposts', '0');
		update_user_meta($user_id, 'lekhook_user_notify_follows', '0');
		update_user_meta($user_id, 'lekhook_user_notify_comments', '0');
	}
}
add_action('user_register', 'lekhook_user_register');


//Process social login user on first login
if (function_exists('wsl_activate')) :
function lekhook_wsl_hook_process_login_after_wp_insert_user($user_id, $provider, $hybridauth_user_profile) {	
	//remove url
	wp_update_user(array('ID' => $user_id, 'user_url' => ''));
}
add_action('wsl_hook_process_login_after_wp_insert_user', 'lekhook_wsl_hook_process_login_after_wp_insert_user', 10, 3);
endif;


//Change social login icons
if (function_exists('wsl_activate')) :
function lekhook_wsl_render_auth_widget_alter_assets_base_url($url) {
	return get_template_directory_uri() . '/img/social/';
}
add_filter('wsl_render_auth_widget_alter_assets_base_url', 'lekhook_wsl_render_auth_widget_alter_assets_base_url');
endif;


//Check and add parent board upon login (in case user did not register through lekhook pro theme register page)
function lekhook_wp_login($user_login, $user) {
	if (!$user) {
...
add_action('wp_login', 'lekhook_wp_login', 10, 2);


//Process social login user on login
if (function_exists('wsl_activate')) :
function lekhook_wsl_hook_process_login_before_wp_safe_redirect($user_id, $provider, $hybridauth_profile) {
	//Check and add parent board upon login (in case user did not register through lekhook pro theme register page) (for social login)
	...
	//fetch wsl avatar
	if (get_user_meta($user_id, 'lekhook_user_avatar', true) == '' && get_user_meta($user_id, 'lekhook_user_avatar', true) != 'deleted' && $imgsrc = get_user_meta($user_id, 'wsl_current_user_image', true)) {
		...
		if (!$error) {
			...
			$attach_id = media_handle_sideload($file_array, 'none', '', array('post_author' => $user_id, 'post_title' => 'Avatar for UserID ' . $user_id)); //use none for $post_id so that image is uploaded to current month/year directory. Else $post_id = this posts page id, which will point to older month/year directory
		...
		if ($error != 'error') {
			update_user_meta($user_id, 'lekhook_user_avatar', $attach_id);
...
add_action('wsl_hook_process_login_before_wp_safe_redirect', 'lekhook_wsl_hook_process_login_before_wp_safe_redirect', 10, 3);
endif;


//Password form
function lekhook_password_form($output) {
	$post = get_post( $post );
	$label = 'pwbox-' . ( empty($post->ID) ? rand() : $post->ID );
	$output = 
	'<form action="' . esc_url( site_url( 'wp-login.php?action=postpass', 'login_post' ) ) . '" class="post-password-form" method="post">
	<p>' . __( 'This content is password protected. To view it please enter your password below:', 'lekhook' ) . '</p>' . 
	'<div class="form-group">' .  
		'<input class="form-control" type="password" name="post_password" id="' . $label . '" value="" />' . 
	'</div>' .
	'<input class="btn btn-success btn-lekhook-custom" type="submit" name="Submit" value="' . esc_attr__( 'Submit', 'lekhook' ) . '" />' .
	'</form>
	';
	return $output;
}
add_filter('the_password_form', 'lekhook_password_form');


//Exclude blog entries from homepage
function lekhook_pre_get_posts($query) {
	if (!is_admin()) {
		//exclude posts from blog
		if (of_get_option('blog_cat_id') && !$query->is_category(lekhook_blog_cats()) && !is_feed()) {
			$query->set('cat', '-' . implode(' -', lekhook_blog_cats()));
		}
...
add_action('pre_get_posts', 'lekhook_pre_get_posts');


//Force second order by ID for meta_value_num
if (!function_exists('lekhook_meta_value_num_orderby')) :
function lekhook_meta_value_num_orderby($orderby) {
	global $wpdb;
...
if (!function_exists('lekhook_comments_orderby')) :
function lekhook_comments_orderby($orderby) {
	global $wpdb;
...
//Pre User Query
function lekhook_pre_user_query($query) {
...
add_action('pre_user_query', 'lekhook_pre_user_query');


//Search in display name
function lekhook_user_search_columns($search_columns, $search){
	if (!in_array('display_name', $search_columns)){
		$search_columns[] = 'display_name';
	}
	return $search_columns;
}
add_filter('user_search_columns', 'lekhook_user_search_columns' , 10, 3);


//Check if user is top user by followers
if (!function_exists('lekhook_top_user_by_followers')) :
function lekhook_top_user_by_followers($user_id) {
	$user_id = (int)$user_id;
	$args = array(
		'order' => 'desc',
		'orderby' => 'meta_value',
...
//Check if user is top user by posts count
if (!function_exists('lekhook_top_user_by_posts')) :
function lekhook_top_user_by_posts($user_id) {
	$user_id = (int)$user_id;
	...
	$posts_count = count_user_posts($user_id) - $blog_post_count;
	...
	$most_posts_pos = array_search($user_id, $top_user_postcount_array);

	if ($most_posts_pos !== false && $posts_count > 0)
		return $most_posts_pos + 1; //return the position e.g 3
	else
		return false;
}
endif;
...
if (!function_exists('lekhook_blog_cats')) :
function lekhook_blog_cats() {
	$blog_cat_id = of_get_option('blog_cat_id');
	$blog_cats = array();
	...
	if ($blog_cat_id) {
		...
		if (get_option('lekhook_blog_subcats')) {
			$blog_cats = array_merge($blog_cats, get_option('lekhook_blog_subcats'));
		}
	}
...
function lekhook_blog_subcats($term_id, $tt_id, $taxonomy) {
	if ($taxonomy == 'category') {
		...
		if ($blog_cat_id) {
			...
			if (!empty($blog_subcats)) {
				update_option('lekhook_blog_subcats', $blog_subcats);
			} else {
				update_option('lekhook_blog_subcats', '');
			}
		}
	}
}
add_action("created_term", 'lekhook_blog_subcats', 10, 3);
add_action("delete_term", 'lekhook_blog_subcats', 10, 3);


//Action: init
function lekhook_init() {
	//Add boards taxonomy
...
add_action('init', 'lekhook_init', 0);


function lekhook_term_link($termlink, $term, $taxonomy) {
	//set board link
...
add_filter('term_link', 'lekhook_term_link', 10, 3);


function lekhook_parse_query($query) {
	//set board query
...
add_action('parse_query', 'lekhook_parse_query');


function lekhook_parse_request($wp) {
...
add_action('parse_request', 'lekhook_parse_request');


//Javascripts
function lekhook_enqueue_scripts() {
    
    wp_enqueue_style('lekhook-boxicons', get_template_directory_uri() . '/boxicons/css/boxicons.min.css');
    
    wp_enqueue_style('lekhook-fontawesome', get_template_directory_uri() . '/css/font-awesome.min.css');
    
	wp_enqueue_style('lekhook-bootstrap', get_template_directory_uri() . '/css/bootstrap.css');
	
	wp_enqueue_style('lekhook-style', get_stylesheet_directory_uri() . '/style.css', array('lekhook-bootstrap'));
	
	if (of_get_option('color_scheme') == 'dark') {
		wp_enqueue_style('lekhook-style-dark', get_template_directory_uri() . '/style-dark.css', array('lekhook-style'));
	}
	...
	if (is_page_template('page_cp_posts.php')) {
		wp_enqueue_script('suggest');
	}
	
	wp_enqueue_script('lekhook_library', get_template_directory_uri() . '/js/lekhook.library.js', array('jquery'), null, true);
	wp_enqueue_script('lekhook_custom', get_template_directory_uri() . '/js/lekhook.custom.js', array('jquery'), null, true);
	...
	if (is_user_logged_in() && !is_page_template('page_cp_boards.php') && !is_page_template('page_cp_boards.php') && !is_page_template('page_cp_login.php') && !is_page_template('page_cp_login_lpw.php') && !is_page_template('page_cp_notifications.php') && !is_page_template('page_cp_posts.php') && !is_page_template('page_cp_register.php') && !is_page_template('page_cp_settings.php') && !is_page_template('page_top_users.php') && !is_404()) {
		if (of_get_option('form_title_desc') != 'separate') {
			if (of_get_option('htmltags') == 'enable') {
				$description_fields = lekhook_wp_editor('post-title');
			} else {
				$description_fields = '<textarea class="form-control" id="post-title" placeholder="' . __('Describe your post...', 'lekhook') .'"></textarea>';
			}
		} else {
			if (of_get_option('htmltags') == 'enable') {
				$description_fields = '<textarea class="form-control" id="post-title" placeholder="' . __('Title...', 'lekhook') . '"></textarea><p></p>' . $description_fields = lekhook_wp_editor('post-content');
			} else {
				$description_fields = '<textarea class="form-control" id="post-title" placeholder="' . __('Title...', 'lekhook') . '"></textarea><p></p><textarea id="post-content" class="form-control" placeholder="' . __('Description...', 'lekhook') . '"></textarea>';
			}
		}
		
		if (of_get_option('posttags') == 'enable') {
			$tags_html = '<div class="input-group"><span class="input-group-addon"><i class="fa fa-tags"></i></span><input class="form-control" type="text" name="tags" id="tags" value="" placeholder="' . __('Tags e.g. comma, separated', 'lekhook') . '" /></div>';
		}
		
		if (of_get_option('price_currency') != '') {
			if (of_get_option('price_currency_position') == 'right') {
				$price_html = '<div class="input-group"><input class="form-control text-right" type="text" name="price" id="price" value="" placeholder="' . __('Price e.g. 23.45', 'lekhook') . '" /><span class="input-group-addon">' . of_get_option('price_currency') . '</span></div>';
			} else {
				$price_html = '<div class="input-group"><span class="input-group-addon">' . of_get_option('price_currency') . '</span><input class="form-control" type="text" name="price" id="price" value="" placeholder="' . __('Price e.g. 23.45', 'lekhook') . '" /></div>';
				}
		}
		
		$dropdown_categories = lekhook_dropdown_categories(__('Category for New Board', 'lekhook'), 'board-add-new-category');
	} else {
		$description_fields = '';
		$tags_html = '';
		$price_html = '';
	...
	$translation_array = array(
		'__allitemsloaded' => __('All items loaded', 'lekhook'),
		'__addanotherpost' => __('Add Another Post', 'lekhook'),
		'__addnewboard' => __('Add new board...', 'lekhook'),
		'__boardalreadyexists' => __('Board already exists. Please try another title.', 'lekhook'),
		'__errorpleasetryagain' => __('Error. Please try again.', 'lekhook'),
		'__cancel' => __('Cancel', 'lekhook'),
		'__close' => __('Close', 'lekhook'),
		'__comment' => __('comment', 'lekhook'),
		'__comments' => __('comments', 'lekhook'),
		'__enternewboardtitle' => __('Enter new board title', 'lekhook'),
		'__Follow' => __('Follow', 'lekhook'),
		'__FollowBoard' => __('Follow Board', 'lekhook'),
		'__Forgot' => __('Forgot?', 'lekhook'),
		'__imagetoosmall' => sprintf(__('Image is too small (min size: %d x %dpx)', 'lekhook'), $minWidth, $minHeight),
		'__incorrectusernamepassword' => __('Incorrect Username/Password', 'lekhook'),
		'__invalidimagefile' => __('Invalid image file. Please choose a JPG/GIF/PNG file.', 'lekhook'),
		'__Likes' => __('Likes', 'lekhook'),
		'__loading' => __('Loading...', 'lekhook'),
		'__Login' => __('Login', 'lekhook'),
		'__NotificationsLatest30' => __('Notifications (Latest 30)', 'lekhook'),
		'__onto' => __('onto', 'lekhook'),
		'__Pleasecreateanewboard' => __('Please create a new board', 'lekhook'),
		'__Pleaseentertitle' => __('Please enter title', 'lekhook'),
		'__Pleaseloginorregisterhere' => __('Please login or register here', 'lekhook'),
		'__Pleasetypeacomment' => __('Please type a comment', 'lekhook'),
		'__or' => __('or', 'lekhook'),
		'__Password' => __('Password', 'lekhook'),
		'__postnedto' => __('Postned to', 'lekhook'),
		'__pleaseenterbothusernameandpassword' => __('Please enter both username and password.', 'lekhook'),
		'__pleaseenterurl' => __('Please enter url', 'lekhook'),
		'__Repost' => __('Repost', 'lekhook'),
		'__Reposts' => __('Reposts', 'lekhook'),
		'__repostnedto' => __('Repostned to', 'lekhook'),
		'__seethispost' => __('See This Post', 'lekhook'),
		'__SeeAll' => __('See All', 'lekhook'),
		'__shareitwithyourfriends' => __('Share it with your friends', 'lekhook'),
		'__SignUp' => __('Sign Up', 'lekhook'),
		'__sorryunbaletofindanypostnableitems' => __('Sorry, unable to find any postnable items.', 'lekhook'),
		'__Unfollow' => __('Unfollow', 'lekhook'),
		'__UnfollowBoard' => __('Unfollow Board', 'lekhook'),
		'__Username' => __('Username or Email', 'lekhook'),
		'__Video' => __('Video', 'lekhook'),
		'__Welcome' => __('Welcome', 'lekhook'),
		'__yourpostispendingreview' => __('Your post is pending review', 'lekhook'),
	...
	wp_localize_script('lekhook_custom', 'obj_lekhook', $translation_array);
}
add_action('wp_enqueue_scripts', 'lekhook_enqueue_scripts');


//Remove style version
function lekhook_style_loader_src($src) {
  global $wp_version;
...
add_filter('style_loader_src', 'lekhook_style_loader_src');
...
if (!function_exists('lekhook_human_time_diff')) :
function lekhook_human_time_diff( $from, $to = '' ) {
	if ( empty( $to ) ) {
		$to = time();
	}
	...
	if ($diff <= 3600) {
		...
		if ($mins == 1) {
			$since = sprintf(__('%s min ago', 'lekhook'), $mins);
		} else {
			$since = sprintf(__('%s mins ago', 'lekhook'), $mins);
		}
		...
		if ($hours == 1) {
			$since = sprintf(__('%s hour ago', 'lekhook'), $hours);
		} else {
			$since = sprintf(__('%s hours ago', 'lekhook'), $hours);
		}
		...
		if ($days == 1) {
			$since = sprintf(__('%s day ago', 'lekhook'), $days);
		} else {
			$since = sprintf(__('%s days ago', 'lekhook'), $days);
		}
	} else {
		$since = get_the_date();
	}
...
function lekhook_feed_content($content) {
	global $post;
	...
	if (lekhook_get_post_board()) {
		global $wp_taxonomies;
		$board_link = home_url('/' . $wp_taxonomies["board"]->rewrite['slug'] . '/' . sanitize_title(lekhook_get_post_board()->name, '_') . '/' . lekhook_get_post_board()->term_id . '/');
		$content_before .= '<p>' . __('Postned onto', 'lekhook') . ' <a href="' . $board_link . '">' . lekhook_get_post_board()->name . '</a></p>';
	}
	
	return ($content_before . $content);
}
add_filter('the_excerpt_rss', 'lekhook_feed_content');
add_filter('the_content_feed', 'lekhook_feed_content');


//Nofollow links
//http://stackoverflow.com/questions/9571210/how-to-set-nofollow-rel-attribute-to-all-outbound-links-in-wordpress-any-plugin
if (!function_exists('lekhook_nofollow_callback')) :
function lekhook_nofollow_callback( $matches ) {
	$link = $matches[0];
	$exclude = '('. home_url() .')';
...
//Comments
if (!function_exists('lekhook_list_comments')) :
function lekhook_list_comments($comment, $args, $depth) {
	global $wp_rewrite;
	...
	<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
		...
		<div class="pull-right<?php if (!is_user_logged_in()) echo ' hide'; ?>"><?php comment_reply_link(array('reply_text' => __('Reply', 'lekhook'), 'login_text' => __('Reply', 'lekhook'), 'depth' => $depth, 'max_depth'=> $args['max_depth'])); ?></div>
		...
		<div class="comment-content">
			...
			<span class="text-muted">&#8226; <?php echo lekhook_human_time_diff(mysql2date('U', get_gmt_from_date(get_comment_date('Y-m-d H:i:s')))); ?></span> <a href="#comment-<?php comment_ID() ?>" title="<?php esc_attr_e('Comment Permalink', 'lekhook'); ?>">#</a> <?php edit_comment_link('e','',''); ?>
			<?php if ($comment->comment_approved == '0') : ?>
			<br /><em><?php _e('Your comment is awaiting moderation.', 'lekhook'); ?></em>
			<?php endif; ?>
...
//Get post price
if (!function_exists('lekhook_get_post_price')) :
function lekhook_get_post_price($show_symbol = true, $post_id = null) {
	...
	return apply_filters('lekhook_get_post_price', $post_price);
}
endif;


//Get post board info
if (!function_exists('lekhook_get_post_board')) :
function lekhook_get_post_board($post_id = null) {
	if (empty($post_id) && isset( $GLOBALS['post'])) {
		global $post;
		$post_id = $post->ID;
	}
...
if (!function_exists('lekhook_get_post_video')) :
function lekhook_get_post_video($url = '') {
	if ($url == '') {
		global $post;
	...
	if (preg_match('%(?:youtube(?:-nocookie)?\.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=)|youtu\.be/)([^"&?/ ]{11})%i', str_replace('&#038;', '&', $url), $videomatch)) {
		...
		$embed_code = apply_filters('lekhook_get_post_video', $embed_code, $url);
	}
...
//Reposts
function lekhook_repost() {
	$nonce = $_POST['nonce'];

	if (!wp_verify_nonce($nonce, 'ajax-nonce'))
		die();

	do_action('lekhook_before_repost', $_POST);

	global $wpdb, $user_ID, $user_identity;
	$original_id  = $_POST['repost_post_id'];
	...
	if (of_get_option('form_title_desc') != 'separate') {
		$duplicate['post_title'] = balanceTags(wp_kses($_POST['repost_title'], $allowed_html), true);
	} else {
		$duplicate['post_title'] = sanitize_text_field($_POST['repost_title']);
	}

	$duplicate['post_content'] = balanceTags(wp_kses($_POST['repost_content'], $allowed_html), true);
	...
	remove_action('save_post', 'lekhook_save_post', 50, 2);
	$duplicate_id = wp_insert_post($duplicate);

	//set board
	$board_add_new = sanitize_text_field($_POST['repost_board_add_new']);
	$board_add_new_category = $_POST['repost_board_add_new_category'];
	$board_parent_id = get_user_meta($user_ID, '_Board Parent ID', true);
	if ($board_add_new !== '') {
		...
		if ($found == '0') {
			$slug = wp_unique_term_slug($board_add_new . '__lekhookboard', 'board'); //append __lekhookboard to solve slug conflict with category and 0 in title
			...
			$repost_board = $new_board_id['term_id'];
		} else {
			$repost_board = $found_board_id;
		}
	} else {
		$repost_board = $_POST['repost_board'];		
	}
	wp_set_post_terms($duplicate_id, array($repost_board), 'board');
	update_user_meta($user_ID, 'lekhook_last_board', $repost_board);

	//set category
	$category_id = get_term_by('id', $repost_board, 'board');
	wp_set_post_terms($duplicate_id, array($category_id->description), 'category');

	//update postmeta for new post
	if ('' == $repost_of_repost = get_post_meta($original_id, '_Original Post ID', true)) { //check if is a simple repost or a repost of a repost
		add_post_meta($duplicate_id, '_Original Post ID', $original_id);
	} else {
		add_post_meta($duplicate_id, '_Original Post ID', $original_id);
		add_post_meta($duplicate_id, '_Earliest Post ID', $repost_of_repost); //the very first post/post		
	}
	...
	//add tags
	wp_set_post_tags($duplicate_id, sanitize_text_field($_POST['repost_tags']));

	//add price	
	if ($_POST['repost_price']) {
		if (strpos($_POST['repost_price'], '.') !== false) {
			$_POST['repost_price'] = number_format($_POST['repost_price'], 2);
		}
		
		add_post_meta($duplicate_id, '_Price',sanitize_text_field($_POST['repost_price']));
	}

	//update postmeta for original post
	$postmeta_repost_count = get_post_meta($original_id, '_Repost Count', true);
	$postmeta_repost_post_id = get_post_meta($original_id, '_Repost Post ID');
	$repost_post_id = $postmeta_repost_post_id[0];

	if (!is_array($repost_post_id))
		$repost_post_id = array();

	array_push($repost_post_id, $duplicate_id);
	update_post_meta($original_id, '_Repost Post ID', $repost_post_id);
	update_post_meta($original_id, '_Repost Count', ++$postmeta_repost_count);
	
	//add to notification center
	if ($user_ID != $original_post_author) {
		$wpdb->query(
			$wpdb->prepare(
				"INSERT INTO " . $wpdb->prefix . "lekhook_notifications (user_id, notification_date, notification_type, notification_from, notification_post_id)
				VALUES (%d, %s, %s, %d, %d)
				"
				, $original_post_author, current_time('mysql'), 'repost', $user_ID, $original_id
			)
		);
		$lekhook_user_notifications_count = get_user_meta($original_post_author, 'lekhook_user_notifications_count', true);
		update_user_meta($original_post_author, 'lekhook_user_notifications_count', ++$lekhook_user_notifications_count);
	}
	
	//email author
	if (get_user_meta($original_post_author, 'lekhook_user_notify_reposts', true) != '' && $user_ID != $original_post_author) {
		$blogname = wp_specialchars_decode(get_option('blogname'), ENT_QUOTES);
		$message = sprintf(__('%s repostned your "%s" post at %s', 'lekhook'), $user_identity, preg_replace('/[\n\r]/', ' ', html_entity_decode(sanitize_text_field(get_the_title($original_id)), ENT_QUOTES, 'UTF-8')), get_permalink($duplicate_id)) . "\r\n\r\n";
		$message .= "-------------------------------------------\r\n";
		$message .= sprintf(__('To change your notification settings, visit %s', 'lekhook'), home_url('/settings/'));
		wp_mail(get_the_author_meta('user_email', $original_post_author), sprintf(__('[%s] Someone repostned your post', 'lekhook'), $blogname), $message);
	}
	...
	do_action('lekhook_after_repost', $duplicate_id);
	
	echo get_permalink($duplicate_id);

	exit;
}
add_action('wp_ajax_lekhook-repost', 'lekhook_repost');

function lekhook_repost_board_populate() {
	global $user_ID;
	echo lekhook_dropdown_boards(null, get_user_meta($user_ID, 'lekhook_last_board', true));
	exit;
}
add_action('wp_ajax_lekhook-repost-board-populate', 'lekhook_repost_board_populate');


//Likes
function lekhook_like() {
	$nonce = $_POST['nonce'];

	if (!wp_verify_nonce($nonce, 'ajax-nonce'))
		die();
		
	do_action('lekhook_before_like', $_POST);

	global $wpdb, $user_ID, $user_identity;
	$post_id = $_POST['post_id'];

	if ($_POST['lekhook_like'] == 'like') {
		...
		$lekhook_user_notifications_count = get_user_meta($_POST['post_author'], 'lekhook_user_notifications_count', true);
		update_user_meta($_POST['post_author'], 'lekhook_user_notifications_count', ++$lekhook_user_notifications_count);

		//email author
		if (get_user_meta($_POST['post_author'], 'lekhook_user_notify_likes', true) != '') {
			$blogname = wp_specialchars_decode(get_option('blogname'), ENT_QUOTES);
			$message = sprintf(__('%s liked your "%s" post at %s', 'lekhook'), $user_identity, preg_replace('/[\n\r]/', ' ', html_entity_decode(sanitize_text_field(get_the_title($post_id)), ENT_QUOTES, 'UTF-8')), get_permalink($post_id)) . "\r\n\r\n";
			$message .= "-------------------------------------------\r\n";
			$message .= sprintf(__('To change your notification settings, visit %s', 'lekhook'), home_url('/settings/'));
			wp_mail(get_the_author_meta('user_email', $_POST['post_author']), sprintf(__('[%s] Someone liked your post', 'lekhook'), $blogname), $message);
		}

		echo $postmeta_count;

	} else if ($_POST['lekhook_like'] == 'unlike') {
		//update postmeta
		$postmeta_count = get_post_meta($post_id, '_Likes Count', true);
		$postmeta_user_id = get_post_meta($post_id, '_Likes User ID');
		$likes_user_id = $postmeta_user_id[0];
	...
	do_action('lekhook_after_like', $post_id, $likes_user_id);
	
	exit;
}
add_action('wp_ajax_lekhook-like', 'lekhook_like');


if (!function_exists('lekhook_liked')) :
function lekhook_liked($post_id) {
	global $user_ID;
	$postmeta_user_id = get_post_meta($post_id, '_Likes User ID');
	$likes_user_id = $postmeta_user_id[0];
...
//Follows
function lekhook_follow() {
	$nonce = $_POST['nonce'];

	if (!wp_verify_nonce($nonce, 'ajax-nonce'))
		die();
	
	do_action('lekhook_before_follow', $_POST);
	...
	if ($_POST['lekhook_follow'] == 'follow') {
		...
		$wpdb->query(
			$wpdb->prepare(
				"INSERT INTO " . $wpdb->prefix . "lekhook_notifications (user_id, notification_date, notification_type, notification_from)
				VALUES (%d, %s, %s, %d)
				"
				, $author_id, current_time('mysql'), 'following', $user_ID
			)
		);
		$lekhook_user_notifications_count = get_user_meta($author_id, 'lekhook_user_notifications_count', true);
		update_user_meta($author_id, 'lekhook_user_notifications_count', ++$lekhook_user_notifications_count);

		//email author
		if (get_user_meta($author_id, 'lekhook_user_notify_follows', true) != '') {
			$blogname = wp_specialchars_decode(get_option('blogname'), ENT_QUOTES);
			$message = sprintf(__('%s is now following you. View %s\'s profile at %s', 'lekhook'), $user_identity, $user_identity, get_author_posts_url($user_ID)) . "\r\n\r\n";
			$message .= "-------------------------------------------\r\n";
			$message .= sprintf(__('To change your notification settings, visit %s', 'lekhook'), home_url('/settings/'));
			wp_mail(get_the_author_meta('user_email', $author_id), sprintf(__('[%s] Someone is following you', 'lekhook'), $blogname), $message);
		}
	} else if ($_POST['lekhook_follow'] == 'unfollow') {		
		//update usermeta following for current user
		$usermeta_following_count = get_user_meta($user_ID, '_Following Count', true);
		$usermeta_following_user_id = get_user_meta($user_ID, '_Following User ID');
		$following_user_id = $usermeta_following_user_id[0];
	...
	do_action('lekhook_after_follow', $board_id, $user_ID, $author_id);
	
	exit;
}
add_action('wp_ajax_lekhook-follow', 'lekhook_follow');


if (!function_exists('lekhook_followed')) :
function lekhook_followed($board_id) {
	global $user_ID;
...
//Ajax comments
function lekhook_ajaxify_comments($comment_ID, $comment_status) {
		...
		if ('spam' !== $comment_status) {
			if ('0' == $comment_status) {
				...
				if ($user_ID != $postdata['post_author']) {
					...
					$lekhook_user_notifications_count = get_user_meta($postdata['post_author'], 'lekhook_user_notifications_count', true);
					update_user_meta($postdata['post_author'], 'lekhook_user_notifications_count', ++$lekhook_user_notifications_count);
				}

				if (get_user_meta($postdata['post_author'], 'lekhook_user_notify_comments', true) != '' && $user_ID != $postdata['post_author']) {
					$message = sprintf(__('%s commented on your "%s" post at %s', 'lekhook'), $user_identity, preg_replace('/[\n\r]/', ' ', html_entity_decode(sanitize_text_field($postdata['post_title']), ENT_QUOTES, 'UTF-8')), get_permalink($postdata['ID'])) . "\r\n\r\n";
					$message .= "-------------------------------------------\r\n";
					$message .= sprintf(__('To change your notification settings, visit %s', 'lekhook'), home_url('/settings/'));
					wp_mail(get_the_author_meta('user_email', $postdata['post_author']), sprintf(__('[%s] Someone commented on your post', 'lekhook'), $blogname), $message);
				}
				...
				if (get_option('comments_notify') && $user_ID != $postdata['post_author']) {
					$admin_message  = sprintf(__('New comment on the post "%s"', 'lekhook'), preg_replace('/[\n\r]/', ' ', html_entity_decode(sanitize_text_field($postdata['post_title']), ENT_QUOTES, 'UTF-8'))) . "\r\n";
					$admin_message .= sprintf(__('Author : %1$s (IP: %2$s , %3$s)', 'lekhook'), $commentdata['comment_author'], $commentdata['comment_author_IP'], $comment_author_domain) . "\r\n";
					$admin_message .= sprintf(__('E-mail : %s', 'lekhook'), $commentdata['comment_author_email']) . "\r\n";
					$admin_message .= sprintf(__('URL    : %s', 'lekhook'), $commentdata['comment_author_url']) . "\r\n";
					$admin_message .= sprintf(__('Whois  : http://whois.arin.net/rest/ip/%s', 'lekhook'), $commentdata['comment_author_IP']) . "\r\n";
					$admin_message .= __('Comment:', 'lekhook') . " \r\n" . $commentdata['comment_content'] . "\r\n\r\n";
					$admin_message .= __('You can see all comments on this post here:', 'lekhook') . " \r\n";
					$admin_message .= get_permalink($postdata['ID']) . "#comments\r\n\r\n";
					$admin_message .= sprintf(__('Permalink: %s', 'lekhook'), get_permalink($postdata['ID']) . '#comment-' . $comment_ID) . "\r\n";
					$admin_message .= sprintf(__('Delete it: %s', 'lekhook'), admin_url("comment.php?action=delete&c=$comment_ID")) . "\r\n";
					$admin_message .= sprintf(__('Spam it: %s', 'lekhook'), admin_url("comment.php?action=spam&c=$comment_ID")) . "\r\n";
					$admin_subject = sprintf(__('[%1$s] Comment: "%2$s"', 'lekhook'), $blogname, preg_replace('/[\n\r]/', ' ', html_entity_decode(sanitize_text_field($postdata['post_title']), ENT_QUOTES, 'UTF-8')));
...
add_action('comment_post', 'lekhook_ajaxify_comments', 500, 2);


//Auto assign board if no board e.g adding post from backend
function lekhook_save_post($post_id, $post) {
	if ($post->post_type != 'post' || $post->post_status != 'publish')
		return;
	
	//Exclude blog category
	$boards = get_the_terms($post_id, 'board');
	if (!$boards && !in_category(lekhook_blog_cats(), $post_id)) {
		...
		if ($found == '0') {
			$slug = wp_unique_term_slug($post_category[0]->cat_name . '__lekhookboard', 'board'); //append __lekhookboard to solve slug conflict with category and 0 in title
...
add_action('save_post', 'lekhook_save_post', 50, 2);


//Clean up when delete post
function lekhook_before_delete_post($post_id) {
	global $wpdb;

	$original_id = get_post_meta($post_id, '_Original Post ID', true);

	if ($original_id == '') { //this is an original post
		//remove instances from repostned postmeta
		$wpdb->query(
			$wpdb->prepare(
		...
		//remove instances from repostned of repostned postmeta
		$wpdb->query(
			$wpdb->prepare(
				"UPDATE $wpdb->postmeta
				SET meta_value = 'deleted'
		...
		$postmeta_repost_post_id = get_post_meta($original_id, '_Repost Post ID');
		$repost_post_id = $postmeta_repost_post_id[0];
		unset($repost_post_id[array_search($post_id, $repost_post_id)]);
		$repost_post_id = array_values($repost_post_id);
	...
	if ($boards && !is_wp_error($boards)) {
		...
		//look for other posts using the same featured image e.g thru repost
	...
	$wpdb->query(
		$wpdb->prepare(
			"DELETE FROM " . $wpdb->prefix . "lekhook_notifications
...
add_action('before_delete_post', 'lekhook_before_delete_post');


//Delete account
function lekhook_delete_account() {
	$nonce = $_POST['nonce'];
...
add_action('wp_ajax_lekhook-delete-account', 'lekhook_delete_account');


//Clean up usermeta & boards when delete user
function lekhook_delete_user($id) {
	global $wpdb;
	...
	//delete avatar
	$user_avatar = get_user_meta($id, 'lekhook_user_avatar', true);
	...
	$user_cover = get_user_meta($id, 'lekhook_user_cover', true);
	
	if ($user_cover != '') {		
		wp_delete_attachment($user_cover, true);
	}
}
add_action('delete_user', 'lekhook_delete_user');


function lekhook_deleted_user($id, $reassign) {
	...
	if (isset($board_parent) && $board_parent['parent'] == '0') {
		...
		if ($reassign === null) { //delete the boards
			...
			foreach ($child_boards as $child_board) {
				...
				$slug = wp_unique_term_slug($child_board_info['name'] . '-@user' . $id . '__lekhookboard', 'board'); //append __lekhookboard to solve slug conflict with category and 0 in title
				wp_update_term(
					$child_board,
					'board',
					array(
...
add_action('deleted_user', 'lekhook_deleted_user', 10, 2);


//Don't delete attachments and posts when deleting users, manual delete by setting status to prune
function lekhook_post_types_to_delete_with_user($post_types_to_delete) {
	unset($post_types_to_delete[array_search('attachment', $post_types_to_delete)]);
	return $post_types_to_delete;
}
add_filter('post_types_to_delete_with_user', 'lekhook_post_types_to_delete_with_user');


//Prune posts
function lekhook_cron_schedules($schedules) {
	$prune_duration = of_get_option('prune_duration') * 60;
	
    $schedules['lekhook_prune'] = array(
...
add_filter('cron_schedules', 'lekhook_cron_schedules');


if (!wp_next_scheduled( 'lekhook_cron_action')) {
    wp_schedule_event(time(), 'lekhook_prune', 'lekhook_cron_action');
}
 
function lekhook_cron_function() {
	global $wpdb;
	...
	$posts = $wpdb->get_results(
		$wpdb->prepare(
			...
			WHERE post_status = 'lekhook_prune'
			LIMIT %d
			"
			, $prune_postnumber
		)
	...
	$orphaned_attachment = $wpdb->get_var(
		...
		AND $wpdb->postmeta.meta_key = 'lekhook_unattached'
...
add_action('lekhook_cron_action', 'lekhook_cron_function');


//Format slug for tags when name is same as board
function lekhook_created_term($term_id, $tt_id, $taxonomy) {
	if ($taxonomy == 'post_tag') {
		$term = get_term($term_id, $taxonomy);
		if (strpos($term->slug, '__lekhookboard') !== false){
			$slug = str_replace('__lekhookboard', '', $term->slug);
			wp_update_term($term_id, $taxonomy, array('slug' => $slug));
		}
	}
}
add_action('created_term', 'lekhook_created_term', 10, 3);


//Change default email
function lekhook_mail_from($email) {
...
add_filter('wp_mail_from', 'lekhook_mail_from');

function lekhook_mail_from_name($name) {
...
add_filter('wp_mail_from_name', 'lekhook_mail_from_name');


//Local avatar
function lekhook_get_avatar($avatar, $id_or_email, $size, $default, $alt) {
	...
	$avatar_id = get_user_meta($id_or_email, 'lekhook_user_avatar', true);
	...
	if ($avatar_id != '' && $avatar_id != 'deleted') {
		...
		if (of_get_option('default_avatar') == '') { 
			...
			if ($size <= 64) {
				$default = get_option('lekhook_avatar_48');
			} else {
				$default = get_option('lekhook_avatar_96');
			}
		}
...
add_filter('get_avatar', 'lekhook_get_avatar', 10, 5);


//Ajax upload avatar
function lekhook_upload_avatar(){
	...
	if ($_FILES) {
		...
		foreach ($_FILES as $file => $array) {
			...
			if (is_wp_error($attach_id)) {
				...
				$user_avatar = get_user_meta($_POST['avatar-userid'], 'lekhook_user_avatar', true);
				if ($user_avatar != '' && $user_avatar != 'deleted')
					wp_delete_attachment($user_avatar, true);

				update_user_meta($_POST['avatar-userid'], 'lekhook_user_avatar', $attach_id);
...
add_action('wp_ajax_lekhook-upload-avatar', 'lekhook_upload_avatar');


//Delete avatar
function lekhook_delete_avatar() {
	$nonce = $_POST['nonce'];

	if (!wp_verify_nonce($nonce, 'ajax-nonce'))
		die();

	$user_avatar = get_user_meta($_POST['id'], 'lekhook_user_avatar', true);
	...
	if ($user_avatar != '' && $user_avatar != 'deleted') {
		...
		wp_delete_attachment($user_avatar, true);
		update_user_meta($_POST['id'], 'lekhook_user_avatar', 'deleted');
	}
	exit;
}
add_action('wp_ajax_lekhook-delete-avatar', 'lekhook_delete_avatar');


//Ajax upload cover
function lekhook_upload_cover(){
	...
	if ($_FILES) {
		foreach ($_FILES as $file => $array) {				
			...
			if (is_wp_error($attach_id)) {
				...
				$user_cover = get_user_meta($_POST['cover-userid'], 'lekhook_user_cover', true);
				if ($user_cover != '')
					wp_delete_attachment($user_cover, true);

				update_user_meta($_POST['cover-userid'], 'lekhook_user_cover', $attach_id);
...
add_action('wp_ajax_lekhook-upload-cover', 'lekhook_upload_cover');


//Delete cover
function lekhook_delete_cover() {
	$nonce = $_POST['nonce'];

	if (!wp_verify_nonce($nonce, 'ajax-nonce'))
		die();

	$user_cover = get_user_meta($_POST['id'], 'lekhook_user_cover', true);
	
	wp_delete_attachment($user_cover, true);
	update_user_meta($_POST['id'], 'lekhook_user_cover', '');
	exit;
}
add_action('wp_ajax_lekhook-delete-cover', 'lekhook_delete_cover');


//**User Control Panel**//

//Add Board/Edit Board
function lekhook_add_board() {
	$nonce = $_POST['nonce'];

	if (!wp_verify_nonce($nonce, 'ajax-nonce'))
		die();
		
	do_action('lekhook_before_add_board', $_POST);
	...
	if ($mode == 'add') {
		...
		if ($found == '0') {
			$slug = wp_unique_term_slug($board_title . '__lekhookboard', 'board'); //append __lekhookboard to solve slug conflict with category and 0 in title
		...
		do_action('lekhook_after_add_board', $new_board_id);
	} else if ($mode == 'edit') {
		...
		do_action('lekhook_after_edit_board', $term_id);
	}
	
	exit;
}
add_action('wp_ajax_lekhook-add-board', 'lekhook_add_board');


//Delete board
function lekhook_delete_board() {
	$nonce = $_POST['nonce'];

	if (!wp_verify_nonce($nonce, 'ajax-nonce'))
		die();
		
	do_action('lekhook_before_delete_board', $_POST);
	...
	do_action('lekhook_after_delete_board', $board_info->term_id);

	echo get_author_posts_url($user_id);
	exit;
}
add_action('wp_ajax_lekhook-delete-board', 'lekhook_delete_board');


//Add post
function lekhook_upload_post(){
    check_ajax_referer('upload_post', 'ajax-nonce');
	
	do_action('lekhook_before_upload_post', $_POST);
	...
	if ($_POST['mode'] == 'computer') {
		...
		do_action('lekhook_after_upload_post_computer', $attach_id);
		echo json_encode($return);
	} else if ($_POST['mode'] == 'web') {
		$url = esc_url_raw($_POST['post_upload_web']);
		...
		do_action('lekhook_after_upload_post_web', $attach_id);
		echo json_encode($return);
	}
	exit;
}
add_action('wp_ajax_lekhook-upload-post', 'lekhook_upload_post');

//Remove %20 from filenames
function lekhook_sanitize_file_name($filename, $filename_raw) {
	$filename = str_replace('%20', '-', $filename);
	return $filename;
}
add_filter('sanitize_file_name', 'lekhook_sanitize_file_name', 1, 2);

//Add post as a wp post
function lekhook_postdata() {
	$nonce = $_POST['nonce'];

	if (!wp_verify_nonce($nonce, 'ajax-nonce'))
		die();

	do_action('lekhook_before_add_post', $_POST);
	...
	wp_set_post_terms($post_id, array($postdata_board), 'board');
	update_user_meta($user_ID, 'lekhook_last_board', $postdata_board);
	...
	delete_post_meta($attachment_id, 'lekhook_unattached');
	...
	do_action('lekhook_after_add_post', $post_id);

	echo get_permalink($post_id);
	exit;
}
add_action('wp_ajax_lekhook-postdata', 'lekhook_postdata');


//Edit post
function lekhook_edit() {
	$nonce = $_POST['nonce'];

	if (!wp_verify_nonce($nonce, 'ajax-nonce'))
		die();
		
	do_action('lekhook_before_edit_post', $_POST);
	...
	do_action('lekhook_after_edit_post', $post_id);
	
	echo get_permalink($post_id);
	exit;
}
add_action('wp_ajax_lekhook-post-edit', 'lekhook_edit');


//Output WP Editor
if (!function_exists('lekhook_wp_editor')) :
function lekhook_wp_editor($editor_id, $post_content = '') {
	$settings = array(
		'textarea_rows' => 2,
		'media_buttons' => false,
		'quicktags' => false,
	...
	$editor_contents .= '<div class="placeholder_description">' . __('Description', 'lekhook') . '</div>';
...
//Replace image
function lekhook_replace_image(){
...
add_action('wp_ajax_lekhook-replace-image', 'lekhook_replace_image');


//Delete post
function lekhook_delete_post() {
	$nonce = $_POST['nonce'];
	...
	echo get_author_posts_url($post_author) . '?view=posts';
	exit;
}
add_action('wp_ajax_lekhook-delete-post', 'lekhook_delete_post');


//Output categories dropdown
if (!function_exists('lekhook_dropdown_categories')) :
function lekhook_dropdown_categories($show_option_none, $name, $selected = '') {
	if (of_get_option('blog_cat_id')) {
		return apply_filters('lekhook_dropdown_categories', wp_dropdown_categories(array('hierarchical' => true, 'show_option_none' => $show_option_none, 'exclude_tree' => of_get_option('blog_cat_id') . ',1', 'hide_empty' => 0, 'name' => $name, 'orderby' => 'name', 'selected' => $selected, 'echo' => 0, 'class' => 'form-control')));
	} else {
		return apply_filters('lekhook_dropdown_categories', wp_dropdown_categories(array('hierarchical' => true, 'show_option_none' => $show_option_none, 'exclude' => '1', 'hide_empty' => 0, 'name' => $name, 'orderby' => 'name', 'selected' => $selected, 'echo' => 0, 'class' => 'form-control')));
	}
}
endif;
...
if (!function_exists('lekhook_dropdown_boards')) :
function lekhook_dropdown_boards($user_id = null, $selected = '') {
	if (!$user_id)
	...
	if (is_array($board_children_count) || $board_children_count == 0) {
		return apply_filters('lekhook_dropdown_boards', '<span id="noboard">' . 
		wp_dropdown_categories(array('taxonomy' => 'board', 'parent' => $board_parent_id, 'hide_empty' => 0, 'name' => 'board', 'hierarchical' => true, 'echo' => 0, 'selected' => $selected, 'show_option_none' => __('Add a new board first...', 'lekhook'), 'class' => 'form-control' )) . 
		'</span>');
	} else {
		return apply_filters('lekhook_dropdown_boards', wp_dropdown_categories(array('taxonomy' => 'board', 'parent' => $board_parent_id, 'hide_empty' => 0, 'name' => 'board', 'hierarchical' => true, 'orderby'=> 'name', 'order' => 'ASC', 'echo' => 0, 'selected' => $selected, 'class' => 'form-control')));
	}
}
endif;
...
function lekhook_post_email() {
	...
	if ($post_id) { //from single-post.php
		$message = sprintf(__('Hi %s', 'lekhook'), $recipient_name) . "\r\n\r\n";
		$message .= sprintf(__('%s wants to share "%s" with you.', 'lekhook'), $user_identity, preg_replace('/[\n\r]/', ' ', html_entity_decode(sanitize_text_field(get_the_title($post_id)), ENT_QUOTES, 'UTF-8'))) . "\r\n\r\n";
		if ($recipient_message !='') {
			$message .= sprintf(__('%s said, "%s".', 'lekhook'), $user_identity, $recipient_message) . "\r\n\r\n";
		}
		$message .= sprintf(__('View post at %s', 'lekhook'), get_permalink($post_id)) . "\r\n\r\n";
		$message .= sprintf(__('View %s\'s profile at %s', 'lekhook'), $user_identity, home_url('/') . $wp_rewrite->author_base. '/' . $user_info->user_nicename . '/') . "\r\n\r\n";

		wp_mail($recipient_email, sprintf(__('%s wants to share a post with you from %s', 'lekhook'), $user_identity, $blogname), $message);
	}
	
	if ($board_id) { //from taxonomy-board.php
		$board_info = get_term_by('id', $board_id, 'board');
		$message = sprintf(__('Hi %s', 'lekhook'), $recipient_name) . "\r\n\r\n";
		$message .= sprintf(__('%s wants to share "%s" with you.', 'lekhook'), $user_identity, sanitize_text_field($board_info->name)) . "\r\n\r\n";
		if ($recipient_message !='') {
			$message .= sprintf(__('%s said, "%s".', 'lekhook'), $user_identity, $recipient_message) . "\r\n\r\n";
		}
		$message .= sprintf(__('View board at %s', 'lekhook'), home_url('/' . $wp_taxonomies["board"]->rewrite['slug'] . '/' . sanitize_title($board_info->name, '_') . '/') . $board_info->term_id) . '/' . "\r\n\r\n";
		$message .= sprintf(__('View %s\'s profile at %s', 'lekhook'), $user_identity, home_url('/') . $wp_rewrite->author_base. '/' . $user_info->user_nicename . '/') . "\r\n\r\n";
		
		wp_mail($recipient_email, sprintf(__('%s wants to share a board with you from %s', 'lekhook'), $user_identity, $blogname), $message);
	}
	exit;
}
add_action('wp_ajax_lekhook-post-email', 'lekhook_post_email');


//Report post
function lekhook_post_report() {
	$nonce = $_POST['nonce'];
	...
	if ($user_ID) {
		$message = sprintf(__('User(%s) reported the "%s" post.', 'lekhook'), $user_login, preg_replace('/[\n\r]/', ' ', html_entity_decode(sanitize_text_field(get_the_title($post_id)), ENT_QUOTES, 'UTF-8'))) . "\r\n";
	} else {
		$message = sprintf(__('An unregistered user reported the "%s" post.', 'lekhook'), preg_replace('/[\n\r]/', ' ', html_entity_decode(sanitize_text_field(get_the_title($post_id)), ENT_QUOTES, 'UTF-8'))) . "\r\n";
	}
	$message .= sprintf(__('Message: %s', 'lekhook'), $report_message) . "\r\n";
	$message .= sprintf(__('View post at %s', 'lekhook'), get_permalink($post_id)) . "\r\n\r\n";
	wp_mail(get_option('admin_email'), sprintf(__('[%s] Someone reported a post', 'lekhook'), $blogname), $message);
	exit;
}
add_action('wp_ajax_lekhook-post-report', 'lekhook_post_report');
add_action('wp_ajax_nopriv_lekhook-post-report', 'lekhook_post_report');


//Custom usermeta - allow admin to override email verification
function lekhook_edit_user_profile($user) {
...
add_action('edit_user_profile', 'lekhook_edit_user_profile');


function lekhook_edit_user_profile_update($user_id) {
	if (!$_POST['_Verify_Email']) {
		delete_user_meta($user_id, '_Verify Email');
	}
}
add_action('edit_user_profile_update', 'lekhook_edit_user_profile_update');


//Setup theme for first time
function lekhook_setup() {
	global $wpdb;
	$lekhook_version = get_option('lekhook_version');
	if (!$lekhook_version) {
		//setup database table
		$sql = "CREATE TABLE IF NOT EXISTS `" . $wpdb->prefix . "lekhook_notifications` (
		...
		add_post_meta($pageid, '_wp_page_template', 'page_videos.php');
		...
		$pageid = wp_insert_post($page);
		add_post_meta($pageid, '_wp_page_template', 'page_shopping.php');
        
		//setup pages
		$page= array(
			'post_title' => 'Add or Edit Board',
			'post_name' => 'boards',
			'post_author' => 1,
			'post_status' => 'publish',
			'post_type' => 'page',
		...
		$pageid = wp_insert_post($page);
		add_post_meta($pageid, '_wp_page_template', 'page_cp_posts.php');
		...
		//setup user accounts
		$lekhook_users = get_users('orderby=ID');
		foreach ($lekhook_users as $user) {
			$board_parent_id = get_user_meta($user->ID, '_Board Parent ID', true);
			...
			if ($board_parent_id == '') {
				...
				update_user_meta($user->ID, '_Board Parent ID', $board_id['term_id']);
				update_user_meta($user->ID, 'lekhook_user_notify_likes', '1');
				update_user_meta($user->ID, 'lekhook_user_notify_reposts', '1');
				update_user_meta($user->ID, 'lekhook_user_notify_follows', '1');
				update_user_meta($user->ID, 'lekhook_user_notify_comments', '1');
			}
		}
	
		update_option('lekhook_version', '2.1');
		add_action('admin_notices', 'lekhook_admin_notices');
	} else if (floatval($lekhook_version) <= 2.0) {
		update_option('lekhook_version', '2.1');
	}
}
add_action('admin_init', 'lekhook_setup');

if (EMPTY_TRASH_DAYS != 0) {
	add_action('admin_notices', 'lekhook_admin_notices');
}

function lekhook_admin_notices() {
...
//Setup Guide
function lekhook_setup_guide() {
	if (function_exists('add_options_page'))
		add_theme_page('Theme Installation', 'Theme Installation', 'edit_theme_options', 'theme_installation', 'lekhook_setup_guide_page');
}

function lekhook_setup_guide_page() {
    ...
    <table class="form-table"><tr><th>
		...
		<ol>
			...
			<li>
				...
				<ul>
					<li>Custom Structure = /post/%post_id%/</li>
				</ul>
			</li>
...
add_action('admin_menu', 'lekhook_setup_guide');
?>