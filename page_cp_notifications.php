...
$notifications_count = $wpdb->get_var(
	$wpdb->prepare(
		"SELECT COUNT(*)
		FROM " . $wpdb->prefix . "lekhook_notifications
		WHERE user_id = %d
...
$notifications_count_unread = get_user_meta($user_ID, 'lekhook_user_notifications_count', true);
...
<div class="container">
	<div class="row">
		...
		<div id="user-notifications" class="col-sm-8 usercp-wrapper">
			<h1><?php the_title(); ?></h1>
			<p class="help-block"><em><?php _e('Only notifications from last 30 days are available for viewing', 'lekhook'); ?></em></p>
			<table id="user-notifications-table" class="table">
				<?php if ($notifications_count == 0) { ?>
					<tr class="info">
						<td class="notifications-none"><div class="text-center"><strong><?php _e('No Notifications Yet.', 'lekhook'); ?></strong></div></td>
					</tr>
				<?php
				} else {
				...
				?>
					<tr class="notifications-wrapper<?php if ($count <= $notifications_count_unread) echo ' notifications_unread'; ?>">
						...
						<td class="notifications_msg text-left">
							<a href="<?php echo home_url('/' . $wp_rewrite->author_base . '/') . $user_info->user_nicename; ?>/"><?php echo $user_info->display_name; ?></a> 
							<?php 
							switch ($notification->notification_type)
							{
							case "like":
								_e('liked your', 'lekhook');
								echo ' <a href="' . get_permalink($notification->notification_post_id) . '">' . __('post', 'lekhook') . '</a>';
								break;
							case "repost":
								_e('repostned your', 'lekhook');
								echo ' <a href="' . get_permalink($notification->notification_post_id) . '">' . __('post', 'lekhook') . '</a>';
								break;
							case "following":
								_e('is following you', 'lekhook');
								break;
							case "comment":
								_e('commented on your', 'lekhook');
								echo ' <a href="' . get_permalink($notification->notification_post_id) . '">' . __('post', 'lekhook') . '</a>';
								break;
							}							
							?>
							<br />
							<?php echo lekhook_human_time_diff(strtotime($notification->notification_date),  current_time('timestamp')); ?>
						</td>
				...
				<div id="navigation">
					<ul class="pager">				
						...
						<li id="navigation-previous">
							<a href="?pnum=<?php echo $pnum-1; ?>"><?php _e('&laquo; Previous', 'lekhook') ?></a>
						</li>
						<?php } ?>
						
						<?php if ($maxpage != 1 && $maxpage != $pnum) { ?>
						<li id="navigation-next">
							<a href="?pnum=<?php echo $pnum+1; ?>"><?php _e('Next &raquo;', 'lekhook') ?></a>
						</li>
						<?php } ?>
					</ul>
				</div>
...
<?php
update_user_meta($user_ID, 'lekhook_user_notifications_count', '0');
get_footer();
?>