<div id="post-masonry" class="container-fluid">
	...
	if (!empty($tags_related)) {
		...
		//if no posts with same tags, find posts with same category
	...
	<?php if (have_posts()) { ?>
		...
		<h3 class="text-center"><?php _e('Related Posts', 'lekhook'); ?></h3>
	<?php } ?>
	...
	<div id="navigation">
		<ul class="pager">			
			...
			<li id="navigation-previous">
				<a href="<?php echo get_permalink() . '?pnum=' . ($pnum-1); ?>"><?php _e('&laquo; Previous', 'lekhook') ?></a>
			</li>
			<?php } ?>
			
			<?php if ($maxpage != 1 && $maxpage != $pnum) { ?>
			<li id="navigation-next">
				<a href="<?php echo get_permalink() . '?pnum=' . ($pnum+1); ?>"><?php _e('Next &raquo;', 'lekhook') ?></a>
			</li>
			<?php } ?>
		</ul>
	</div>
