...
<div class="container-fluid">
	...
	<div id="userbar" class="row">
		<ul class="nav">
			...
			<li><a href="<?php echo get_category_link($category->cat_ID); ?>"><?php _e('Category', 'lekhook'); ?><br /><strong><?php echo $category->name; ?></strong></a></li>
			<li><?php _e('Posts', 'lekhook'); ?><br /><strong><?php echo $board_info->count; ?></strong></li>
			<li>
				<?php if ($board_user != $user_ID) { ?>
				<span class="undisable_buttons">
					<button class="btn btn-success btn-sm follow lekhook-follow<?php if ($followed = lekhook_followed($board_info->term_id)) { echo ' disabled'; } ?>" data-author_id="<?php echo $board_user; ?>" data-board_id="<?php echo $board_info->term_id;  ?>" data-board_parent_id="<?php echo $board_info->parent; ?>" type="button"><?php if (!$followed) { _e('Follow Board', 'lekhook'); } else { _e('Unfollow Board', 'lekhook'); } ?></button>
				</span>
				<?php } ?>
				...
				<div class="lekhook-share btn-group">
					...
					<ul class="dropdown-menu">
						...
						<li><a href="" onclick="window.open('https://plus.google.com/share?url=<?php echo home_url('/' . $wp_taxonomies["board"]->rewrite['slug'] . '/' . sanitize_title($board_info->name, '_') . '/') . $wp_query->query['board'] . '/'; ?>', 'gplus-share-dialog', 'width=626,height=500'); return false;"><i class="fa fa-google-plus-square fa-lg fa-fw text-danger"></i> <?php _e('Share on Google+', 'lekhook'); ?></a></li>	
						<li><a href="" onclick="window.open('http://postterest.com/post/create/button/?url=<?php echo rawurlencode(home_url('/' . $wp_taxonomies["board"]->rewrite['slug'] . '/' . sanitize_title($board_info->name, '_') . '/') . $wp_query->query['board']. '/'); ?>&amp;media=<?php echo rawurlencode($imgsrc[0]); ?>&amp;description=<?php echo rawurlencode($board_info->name); ?>', 'postterest-share-dialog', 'width=626,height=500'); return false;"><i class="fa fa-postterest-square fa-lg fa-fw text-danger"></i> <?php _e('Share on Postterest', 'lekhook'); ?></a></li>
					</ul>
				</div>
					
				<?php if ($board_info->parent && ($board_user == $user_ID || current_user_can('edit_others_posts'))) { ?>
				<button class="btn btn-success btn-sm edit-board follow" onclick="window.location='<?php echo home_url('/boards/?i=') . $board_info->term_id; ?>'" type="button"><?php _e('Edit Board' , 'lekhook'); ?></button>
				<?php } ?>
			</li>
		</ul>
	</div>
